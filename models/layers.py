...
try:
    ...
    # Fallback to FlashAttention 2; if unavailable, define a PyTorch SDPA fallback.
    try:
        from flash_attn import flash_attn_func  # type: ignore[import]
    except ImportError:
        def flash_attn_func(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, causal: bool = False):
            """Pure PyTorch fallback using scaled_dot_product_attention.

            Expects q, k, v shapes: [batch, seq_len, num_heads, head_dim].
            ...
            # Use torch's SDPA; is_causal handles causal masking if requested
            out = F.scaled_dot_product_attention(q_, k_, v_, attn_mask=None, dropout_p=0.0, is_causal=causal)
