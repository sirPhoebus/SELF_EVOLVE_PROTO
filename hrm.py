...
from models.evolve.pipeline import SelfEvolvePipeline
...
class HierarchicalReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        ...
        if self.config.puzzle_emb_ndim > 0:
            ...
            embedding = torch.cat((puzzle_embedding.reshape(-1, self.puzzle_emb_len, self.config.hidden_size), embedding), dim=-2)
...
class HierarchicalReasoningModel_ACTV1(nn.Module):
    ...
    def __init__(self, config_dict: dict):
        ...
        self._evolver: Optional[SelfEvolvePipeline] = None
    ...
    def forward(self, carry: HierarchicalReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[HierarchicalReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        new_current_data = {k: torch.where(carry.halted.reshape((-1, ) + (1, ) * (batch[k].ndim - 1)), batch[k], v) for k, v in carry.current_data.items()}
    ...
    def self_evolve(
        ...
        *,
        ...
        """Mine arXiv, extract concepts, propose HRM config updates, optionally return new config.

        If dry_run=True, returns a report containing papers, concepts, actions, rationale, and old_config.
        If dry_run=False, also includes a suggested new_config (not applied automatically).
        """
        if self._evolver is None:
            self._evolver = SelfEvolvePipeline()

        report = self._evolver.run(
            self,
